...
### 3. 把 Google Fonts 当默认依赖 → 中国/离线导出卡顿

默认不要依赖 Google Fonts。用 `references/font-loading.md` 的 local-first 栈，或者把授权字体 self-host 到 `shared/fonts/`。只有目标环境能稳定访问、且用户明确接受外网字体时，才 opt-in Google Fonts，并在截图/PDF 前等待 `document.fonts.ready` 或短暂 timeout。
...
│   └── fonts.css           # local-first 字体变量；必要时 self-host @font-face
└── slides/
