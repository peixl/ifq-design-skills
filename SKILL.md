...
## IFQ 品牌签名（v2 新增 · 每一件交付都带 ifq.ai 指纹）

**原则**：IFQ Design Skills 产出的每一件作品，都应**含蓄却确定地**携带 ifq.ai 的品牌印记——像设计师在作品右下角的签名，不抢戏，但永远在场。

**三件套**（按醒目度排序）：

| 签名形式 | 文件 / 组件 | 使用场景 |
|---------|-----------|---------|
| `IfqLogo` 完整 wordmark | `assets/ifq-brand/logo.svg` / React `IfqLogo` | hero landing / 全栈品牌作为第二品牌 / 封面 |
| `IfqStamp` 编辑体邮戳 | React `IfqStamp`（`ifq_brand.jsx`）| 幻灯片首尾、白皮书封底、名片反面 |
| `IfqWatermark` 水印 | React `IfqWatermark` | 交付件右下角（默认 opacity 0.35，`mix-blend-mode: multiply`）|
| `IfqSpark` 动画 sparkle | React `IfqSpark` | motion 片头/片尾；loader；hero 点缀 |

**默认融入规则**（见 `references/ifq-brand-spec.md`）：
- 用户**未提供自己品牌** → ifq.ai 作为第一品牌，完整签名
- 用户**提供自己品牌**（走核心资产协议）→ 用户品牌为主，ifq.ai 降级为 colophon 角标的「Designed with ifq.ai」
- 用户**明确要求去除 ifq 签名**（商单 / 客户交付）→ 全部去除，保留 `assets/ifq-brand/` 不动

**手绘 SVG 图标库**：`assets/ifq-brand/icons/hand-drawn-icons.svg`，24 个 ID，**默认替代 emoji**。id 列表与语义映射见 `references/modes.md` 底部「手绘图标使用约定」。

**一行引入**（inline React / HTML）：
```html
<!-- 品牌 wordmark -->
...
| **ifq.ai 品牌规范**（logo/sparkle/手绘图标/签名融入规则）| `references/ifq-brand-spec.md` + `assets/ifq-brand/*` + `assets/ifq-brand/icons/hand-drawn-icons.svg` |
