...
## Agent runtime quickref（多 runtime 一行表）

详细工具映射 / 安装命令见 [`references/agent-compatibility.md`](references/agent-compatibility.md)；下表是 agent 加载本 skill 后可直接照抄的最小信息：

| Runtime | 推荐安装路径 | 加载方式 | 核心动词 → 工具 |
|---------|------------|---------|----------------|
...
| **Codex CLI (OpenAI)** | `~/.codex/skills/ifq-design-skills` | 仓库根 `AGENTS.md` 自动注入 | read/write=apply_patch · run=shell |
| **OpenCode (sst)** | 仓库根（已内置 `AGENTS.md`）| `AGENTS.md` 自动注入 | read=read · write=write · run=bash |
| **OpenClaw** | `~/.openclaw/skills/ifq-design-skills` | `plugins.allow` + gateway 重启 | read=filesystem/read · write=filesystem/write · run=shell/exec |
...
| **CodeBuddy (Tencent)** | 仓库根（自动读 `AGENTS.md`）| 聊天里 `@SKILL.md` 显式 pin | read=@file/Read · write=Apply · run=terminal |
| **Generic fallback** | 任何有 fs + shell 的 agent | 指向 `SKILL.md` 即可 | 见 [`references/agent-compatibility.md`](references/agent-compatibility.md) §8 |

## ⚡ 最短执行协议（Fast Path）

> Cheat Sheet 已经讲清「what / when / how」三件事。本节补完 agent 真正动手前剩下的字段：输入、输出、品牌融合、路由顺序、验证硬闸。
...
- **Verification 硬闸**：默认 lite（`npm run verify:lite` + `npm run preview`，零依赖）。其他硬闸：App 原型 ≥ 1 个可点击交互；动画 60fps + BGM fade + 文件体积；Deck 导出 PDF 页数 = HTML slides 数。
- **Routing**：模式触发 > 设计方向顾问 Fallback > Junior Designer 主干。模式触发时**先 Read 对应模板**（[`assets/templates/INDEX.json`](assets/templates/INDEX.json) → 对应 html），再 fork-and-fill。**禁止从白纸开始**。
- **Smoke test**：`npm run smoke` 60 秒验证 skill 完整性（模板索引、identity toolkit、icon sprite、references 路由、脚本语法、placeholder 不泄漏）。
